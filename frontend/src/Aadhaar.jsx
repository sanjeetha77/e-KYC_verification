...
import { useSearchParams } from 'react-router-dom'; // [Added] To get userId from URL
...
// Main Aadhaar Component
const Aadhaar = () => {
    // [Added] Get userId from the URL (e.g., ?userId=123)
    ...
    const handleCapture = () => {
        if (videoRef.current && canvasRef.current) {
            ...
            if (webcamStep === 1) {
                setCapturedFrontImage(imageDataURL);
            } else if (webcamStep === 2) {
                setCapturedBackImage(imageDataURL);
            }
        }
    };

    const handleUseCapturedPhoto = (side) => {
        if (side === 'front' && capturedFrontImage) {
            const file = dataURLtoFile(capturedFrontImage, 'aadhar_front.jpeg');
            setFrontFile(file);
            setWebcamStep(2); 
            setCapturedFrontImage(null);
        } else if (side === 'back' && capturedBackImage) {
            const file = dataURLtoFile(capturedBackImage, 'aadhar_back.jpeg');
            setBackFile(file);
            stopWebcam();
            setView('upload'); 
        }
    ...
    const handleSubmit = async (e) => {
        ...
        // [Added] Check if userId exists
        if (!userId) {
            setError('Invalid Session: User ID missing from URL. Please use the link sent to your email.');
            return;
        }

        const frontFileToSubmit = frontFile;
        const backFileToSubmit = backFile;

        if (!frontFileToSubmit) {
            setError('Please upload the Aadhaar front image.');
            return;
        }
        ...
        formData.append('id_card_photo', frontFileToSubmit);
        formData.append('id_type', 'aadhar');
        // [Added] Send userId to backend
        formData.append('userId', userId); 
        if (backFileToSubmit) formData.append('id_card_back', backFileToSubmit);
    ...
    const captureAndSend = async () => {
        ...
        if (!videoRef.current || !streamRef.current) {
            setMatchMessage("❌ Camera not started.");
            return;
        }
        if(matching) return;
        ...
        canvas.toBlob(async (blob) => {
            ...
            // [Added] Send userId to backend (Backend now uses userId to find the session)
            formData.append("userId", userId); 
            ...
            try {
                ...
                const faceMatchResult = { 
                    ...
                    status: matchStatus, 
                    message: `Face Match Score: ${apiData.score || 'N/A'}` // Updated key to 'score' based on app.py
                };
        ...
        return (
            <div style={styles.body}>
                <div style={styles.container}>
                    ...
                    <p style={styles.processingText}>Processing Aadhaar document and extracting face...</p>
                </div>
            </div>
        );
    }
    ...
    if (view === 'webcam') {
        const currentSide = webcamStep === 1 ? 'Front' : 'Back';
        const capturedImage = webcamStep === 1 ? capturedFrontImage : capturedBackImage;
        const currentFileName = currentSide.toLowerCase();
        
        return (
            <div style={styles.body}>
                <div style={styles.container}>
                    <header style={styles.header}>
                        <div style={styles.backLink} onClick={stopWebcam}>
                        ...
                        <div style={styles.headerTitle}>
                            <h1 style={styles.headerTitleH1}>Capture Aadhaar {currentSide}</h1>
                        </div>
                        <div></div>
                    </header>
                    <div style={styles.webcamContainer}>
                        {capturedImage ? (
                            <>
                                <img src={capturedImage} alt={`Captured ${currentSide}`} style={styles.previewImage} />
                                <button style={{...styles.submitButton, marginTop: '1rem'}} onClick={() => handleUseCapturedPhoto(currentFileName)}>
                                    Use this Photo
                                </button>
                                <button style={styles.cameraButton} onClick={() => webcamStep === 1 ? setCapturedFrontImage(null) : setCapturedBackImage(null)}>
                                    Re-take
                                </button>
                            </>
                        ) : (
    ...
    if (view === 'live-match') {
        ...
        return (
            <div style={styles.body}>
                <div style={styles.container}>
                    <header style={styles.header}>
                        <div style={styles.backLink} onClick={() => {stopMatchCamera(); setView('upload');}}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M15.8333 10H4.16666" stroke="#344054" strokeWidth="1.67" strokeLinecap="round" strokeLinejoin="round" />
                                <path d="M9.99999 15.8334L4.16666 10.0001L9.99999 4.16675" stroke="#344054" strokeWidth="1.67" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
            ...
            <div style={styles.container}>
                ...
                <main style={styles.mainContent}>
                        ...
                        <ol style={styles.instructionList}>
                            <li style={styles.instructionListItem}>
                                ...
                                <div style={styles.instructionText}>
                                    ...
                                    <p style={styles.instructionTextP}>Ensure your Aadhaar card is clearly visible with all text readable</p>
                                </div>
                            </li>
                            <li style={styles.instructionListItem}>
                                <div style={styles.instructionNumber}>2</div>
                                <div style={styles.instructionText}>
                                    <h3 style={styles.instructionTextH3}>Capture Front and Back</h3>
                                    <p style={styles.instructionTextP}>You must upload or capture both the front and the back of your Aadhaar card</p>
                                </div>
                            </li>
                            <li style={styles.instructionListItem}>
                    ...
                    <section style={styles.uploadSection}>
                        <h2 style={styles.uploadSectionH2}>Upload Your Aadhaar Document</h2>
                        ...
                        <div style={styles.dropZoneContainer}>
                                ...
                                >
                                    ...
                                    <p style={styles.dropZoneP}>
                                        <span style={styles.uploadText}>
                                            {frontFile ? frontFile.name : 'Click to upload'}
                                        </span>
                                        <span style={styles.uploadTextSpan}> Aadhaar Front</span>
                                    </p>
                            ...
                            <label htmlFor="file-upload-back">
                                ...
                                >
                                    ...
                                    <p style={styles.dropZoneP}>
                                        <span style={styles.uploadText}>
                                            {backFile ? backFile.name : 'Click to upload'}
                                        </span>
                                        <span style={styles.uploadTextSpan}> Aadhaar Back (Optional)</span>
                                    </p>
                                    <p style={{ ...styles.dropZoneP, ...styles.fileSpecs }}>We accept JPG/PNG, max 5MB</p>
                                </div>
                            </label>
                        ...
                        <div style={{...styles.separator, paddingLeft: 0}}>
                            <div style={{flexGrow: 1, height: '1px', backgroundColor: '#e0eaff', margin: '0 1rem'}}></div>
                            or
                            <div style={{flexGrow: 1, height: '1px', backgroundColor: '#e0eaff', margin: '0 1rem'}}></div>
                        </div>

                        <button style={styles.cameraButton} onClick={startWebcamCapture}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        {error && <p style={{color: styles.failed.color, marginTop: '1rem', textAlign: 'center'}}>{error}</p>}
                    </section>
                </main>
            </div>
        </div>
